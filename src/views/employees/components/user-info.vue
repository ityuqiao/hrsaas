...
import { getStaffDetailsInfo } from '@/api/user.js'
  ...
  methods: {
    ...
    async getPersonalDetail() {
      ...
      if (this.formData.staffPhoto) {
        this.$refs.userPhoto.FileList = [{ url: this.formData.staffPhoto, upload: true }]
      }
      // console.log(this.$refs.userPhoto.FileList)
      // console.log(this.formData)
      // console.log(this.userId)
    },
    async saveUser() {
      if (this.$refs.StaffPhoto.showProgress) {
        return this.$message.error('图片还未上传完成')
      }
      if (this.$refs.StaffPhoto.FileList.length === 1) {
    ...
    async savePersonal() {
      if (this.$refs.userPhoto.showProgress) {
        return this.$message.error('图片还未上传完成')
      }
      if (this.$refs.userPhoto.FileList.length === 1) {
        this.formData.staffPhoto = this.$refs.userPhoto.FileList[0].url
      }
      await updatePersonal(this.formData)
      this.$message.success('保存用户个人基础信息成功')
      this.getPersonalDetail()
      // console.log(11)
    }
  }
